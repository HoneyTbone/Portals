...
{
	internal static class InspectorContents
	{
		...
		public static IEnumerable<(Editor, IMGUIContainer)> GetComponentHeaderElementsFromInspectorOrPropertyEditorWindow(Editor gameObjectEditor)
		{
			...
			var propertyWindow = GetPropertyEditorWindow(gameObjectEditor);
			if(propertyWindow != null)
			{
				return GetAllComponentHeaderElements(propertyWindow);
			}

			return Enumerable.Empty<(Editor, IMGUIContainer)>();
		}

		public static (Editor, IMGUIContainer)? GetComponentHeaderElementFromPropertyEditorOf(Editor componentEditor)
		{
		...
		private static EditorWindow GetPropertyEditorWindow(Editor componentOrGameObjectEditor)
		{
			...
			for(int i = allPropertyEditors.Count - 1; i >= 0; i--)
			{
				...
				if(ContainsEditor(propertyEditor.rootVisualElement, componentOrGameObjectEditor))
				{
		...
		private static bool ContainsEditor(VisualElement parentElement, Editor gameObjectOrComponentEditor)
		{
			return gameObjectOrComponentEditor.target is GameObject
				? ContainsGameObjectEditor(parentElement, gameObjectOrComponentEditor)
				: ContainsComponentEditor(parentElement, gameObjectOrComponentEditor);
		}
		...
		private static bool ContainsComponentEditor(VisualElement parentElement, Editor gameObjectEditor)
		{
			if(parentElement is null)
			{
				return false;
